...
class AdminStats {
  ...
  async run() {
    let content = await axios
      .get(`https://xtools.wmflabs.org/adminstats/th.wikipedia.org/${this.#start}/${this.#end}?format=wikitext&actions=delete|revision-delete|log-delete|restore|re-block|unblock|re-protect|unprotect|rights|merge|import|abusefilter|contentmodel`)
    // cleanup
    content = new TextContent(content.data).content
...
/**
 * @typedef Config Admin Stats Bot configuration
 * @property {string} header Header to be added for each run
 * @property {string} title Page name to save the statistics to
 */
