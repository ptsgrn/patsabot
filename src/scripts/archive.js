...
async function archive() {
    const archiveScriptPath = resolveRelativePath(import.meta.url, '../../python/archive.py');
    try {
        const { stdout, stderr } = spawn(process.env.NODE_ENV === 'development' ? 'python' : '~/pyvenv/bin/python', [archiveScriptPath], {
            env: {
                ...process.env,
                PATSABOT_ARCHIVE_KEY_SALT: credentials?.scripts?.archive?.key_salt ?? '',
