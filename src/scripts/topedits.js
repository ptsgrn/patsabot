...
function generateTable(data) {
    ...
    for (const { name, ...value } of data) {
        if (value.exclude) {
            ...
            content += `\n|-\n| ${count} || [[ผู้ใช้:${name}|${!value.active ? '<span style="color: gray;">' : ''}${name}${!value.active ? '</span>' : ''}]] ${rightTransform(value.group)} || {{sort|${value.editcount.toString()}|[[พิเศษ:เรื่องที่เขียน/${name}|${value.editcount
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ',')}]]}} `;
        }
        count += 1;
  ...
  LIMIT 580;
    ...
    for (let { name, editcount, is_admin, is_bot } of data) {
        result.push({
            ...
            group: [is_admin ? 'sysop' : null, is_bot ? 'bot' : null].filter((v) => v),
