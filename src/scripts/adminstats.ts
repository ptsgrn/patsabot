...
const ELIGIBLE_GROUPS = ["sysop", "bureaucrat", "interface-admin"];
const DISPLAY_GROUPS = ["sysop", "interface-admin", "bureaucrat", "checkuser"];

function formatMediaWikiTimestamp(date: Date) {
  return date.toISOString().replace(/\D/g, "").slice(0, 14);
}

function insertBetween(str: string, anchorStart: string, anchorEnd: string, insertStr: string) {
  ...
  return str.slice(0, startIndex + anchorStart.length) + insertStr + str.slice(endIndex);
}

function userGroupFormatter(groups: string[]) {
...
function getCount(
  ...
  return stats.find((s) => s.type === type && s.action === action)?.[countField] || 0;
}

function formatAdminStatsLine(admin: AdminStats, countField: "count" | "last6MonthsCount") {
  return (
    `|-\n` +
    `| [[Special:Contributions/${admin.name}|${admin.name}]] || ` +
    `${userGroupFormatter(admin.groups)} || ` +
    QUERY_LIST.map((q) => getCount(admin.counts, q.type, q.action, countField)).join(" || ")
  );
}

function adminStatsFromRows(rows: AdminStatsRow[]) {
...
async function queryAdminStats(
  ctx: ScriptContext,
  last6MonthsTimestamp: string,
  nowTimestamp: string,
) {
  const eligibleGroupPlaceholders = ELIGIBLE_GROUPS.map(() => "?").join(", ");
  const displayGroupPlaceholders = DISPLAY_GROUPS.map(() => "?").join(", ");
  const logActionFilter = QUERY_LIST.map(() => "(logging.log_type = ? AND logging.log_action = ?)").join(
    " OR ",
  );
  const logActionValues = QUERY_LIST.flatMap(({ type, action }) => [type, action]);

  const [rows] = await ctx.replica.query<AdminStatsRow[]>(
    `
  ...
  return adminStatsFromRows(rows);
}

async function getAdminStats(ctx: ScriptContext) {
  const last6MonthsDate = new Date();
  last6MonthsDate.setMonth(last6MonthsDate.getMonth() - 6);

  return queryAdminStats(
    ctx,
    formatMediaWikiTimestamp(last6MonthsDate),
    formatMediaWikiTimestamp(new Date()),
  );
}
...
export default defineScript({
  ...
  async beforeRun(ctx) {
    await ctx.replica.init();
  },

  async run(ctx) {
    const adminStats = await getAdminStats(ctx);
    const adminStatsLines = adminStats.map((admin) => formatAdminStatsLine(admin, "count"));
    const adminStatsLast6MonthsLines = adminStats.map((admin) =>
      formatAdminStatsLine(admin, "last6MonthsCount"),
    );

    const finalAllTimeTable = TABLE_HEADER + adminStatsLines.join("\n") + `\n|}`;
    const finalLast6MonthsTable = TABLE_HEADER + adminStatsLast6MonthsLines.join("\n") + `\n|}`;

    let pageContent =
      (await ctx.bot.read("วิกิพีเดีย:ผู้ดูแลระบบ/สถิติ")).revisions?.[0].content || "";
    pageContent = insertBetween(
      pageContent,
