...
async function main() {
    ...
    const scriptFiles = (await readdir(scriptDir)).filter((file) => file.endsWith('.ts'));
    const taskDataList = (await Promise.all(scriptFiles.map(async (file) => {
        const scriptString = await readFile(resolveRelativePath(import.meta.url, `../scripts/${file}`), {
            encoding: 'utf-8',
        }).catch((err) => {
            console.error(err);
            return '';
    ...
    const schedules = taskDataList.map((taskData) => {
        return {
            ...
            command: `node $HOME/bot/src/patsabot/run.js ${taskData.file.replace('.ts', '')}`,
        };
    });
    ...
    const [prePart, _, postPart] = shutoffPageContent.split(/<!-- \/?taskmarker -->/);
