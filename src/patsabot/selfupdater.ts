...
export default async function selfUpdate(req: Request, res: Response, next: NextFunction) {
  if (req.url !== '/hook') return next()
  ...
  const calculatedSig = `sha256=${createHmac('sha256', credentials.githooksecret)
    ...
    .digest('hex')}`
  if (!timingSafeEqual(Buffer.from(calculatedSig), Buffer.from(sig as string))) {
