...
ssh toolforge -t "become patsabot -- toolforge jobs restart bot-scheduled"
```

### Continuous run command

Use this on Toolforge to run the scheduled bot continuously. It will automatically restart if it crashes.:

```bash
toolforge jobs run bot-scheduled --command "/data/project/patsabot/.bun/bin/bun /data/project/patsabot/www/js/src/core/run.ts start --no-dry-run --no-interactive" --image node20 --continuous
```
