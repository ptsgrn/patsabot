...
interface PromptChoiceEntry<T = string> {
  name: string;
  value: string;
  alias: string[];
}
...
export class PromptChoice implements PromptChoiceEntry {
  ...
  isMatch(value: string) {
    return this.value.toLocaleLowerCase() === value || this.alias.includes(value.toLocaleLowerCase());
  }
}
...
export class Input extends ServiceBase {
  ...
  public async prompt<T extends readonly PromptChoiceEntry[]>(
    message: string,
    choices?: T
  ): Promise<T extends readonly PromptChoiceEntry[] ? T[number]['value'] : 'y' | 'n'> {
    ...
    const choicesEntry = effectiveChoices.map(choice => new PromptChoice(choice.name, choice.value, choice.alias));
    while (true) {
      const question = "> " + message + " (" + choicesEntry.map(c => c.value).join('/') + "): ";
      this.log.debug(question);
      if (isOnIacto()) {
        this.log.debug('Sending notification...');
        const stdout = await $`notify-send "${question}" --wait ${{ raw: "--action=" + choicesEntry.map(v => v.name).join(" --action=") }} --app-name="${this.config.bot.username}"`.quiet().text()
        if (stdout) {
          const selectedAction = +stdout.toString().trim();
          this.log.debug('User selected action: ' + selectedAction);
          return choicesEntry[selectedAction]?.value as T extends readonly PromptChoiceEntry[] ? T[number]['value'] : 'y' | 'n';
        }
      }
      ...
      if (answer && choicesEntry.some(choice => choice.isMatch(answer))) {
        return choicesEntry.find(choice => choice.isMatch(answer))?.value as T extends readonly PromptChoiceEntry[] ? T[number]['value'] : 'y' | 'n';
      } else {
...
export class Output extends ServiceBase {
  ...
  */
  ...
  public diff = diff;
  public printDiff(original: string, updated: string, options: {
    ...
    console.log(differences.map(([operation, text]) => {
      const color = operation === 1 ? colors.colorAdd : operation === -1 ? colors.colorRemove : colors.colorChange;
      return (color(text));
    }).join(''));
  }
}