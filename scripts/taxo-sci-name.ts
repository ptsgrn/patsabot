...
export default class TaxonomyAddScientificName extends Bot {
  ...
  async run(): Promise<void> {
    ...
    // }}
    // `, "Achlysiella").content)
    const linkedCategoryWithoutScienctificName = new this.bot.Category("แม่แบบอนุกรมมีหน้าที่ลิงก์แต่ไม่ระบุ scientific name")

    for await (const page of linkedCategoryWithoutScienctificName.membersGen({
      cmnamespace: 10,
    })) {
      this.log.info(`>>> ${"https://th.wikipedia.org/wiki/" + page.title.replaceAll(" ", "_")} <<<`);
  ...
  async processPage(page: ApiPage) {
    ...
    await this.bot.edit(page.title, async ({ content }) => {
      const { ok, error, content: processedContent } = await this.processPageContent(content, scientificName);
      ...
      this.log.info(`Processing ${this.output.color.bgBlueBright(page.title)} with scientific name "${this.output.color.bgBlueBright(scientificName)}"`);
      // console.log(processedContent);
      this.output.printDiff(content, processedContent);

      if (this.cli.opts().auto) {
        this.log.info(`Auto-confirming changes for ${page.title} in ${this.cli.opts().delay} milliseconds`);
        await new Promise(resolve => setTimeout(resolve, this.cli.opts().delay));
      ...
      const result = await this.input.prompt(`Save the changes to "${page.title}"?`, [
        {
          name: "Yes",
          value: "yes",
          alias: ["y"],
  ...
  async processPageContent(content: string, scientificName: string) {
    ...
    let newContent = contentMatch ? contentMatch[1] + `|scientific_name=${scientificName}\n` + contentMatch[2] : content
    const rank = this.getRankFromText(newContent) || ""

    const suggestedDisplayName = await this.suggestDisplayName(newContent, rank);

    if (suggestedDisplayName) {
      const result = await this.input.prompt(`Suggested: "${suggestedDisplayName.linkDisplay}" for ${rank}:"${scientificName}"?`, [
        {
          name: "Yes",
          value: "yes",
          alias: ["y"],
      ...
      if (result === 'custom') {
        const customDisplayName = this.input.ask(`Enter a custom display name for ${rank}:"${scientificName}":`);
        if (!customDisplayName) {
          this.log.warn(`No custom display name provided for ${rank}:"${scientificName}".`);
          return {
            ok: false as const,
            error: "No custom display name provided"
          }
        }
        newContent = this.setDisplayText(newContent, customDisplayName, suggestedDisplayName.linkTarget);
      }

      if (result === "yes") {
        newContent = this.setDisplayText(newContent, suggestedDisplayName.linkDisplay, suggestedDisplayName.linkTarget);
      }
    }
  ...
  async suggestDisplayName(content: string, rank: string) {
    ...
    try {
      ...
      if (await linkTargetPage.isRedirect()) {
        let redirectTarget = await linkTargetPage.getRedirectTarget()
        if (!redirectTarget.startsWith(this.ranks[rank])) {
          this.log.warn(`Redirect target "${redirectTarget}" does not start with rank prefix "${this.ranks[rank]}".`);
        }
      ...
      this.log.error(`Failed to get redirect target for "${linkTarget}":`, error);
      return null;
    }
  }
  ...
  cleanDisplayText(display: string) {
    return Object.entries(this.ranks)
      .reduce((acc, [rank, prefix]) => {
        return acc.replace(new RegExp(`^${prefix}`, 'i'), '').trim();
      }, display).replace(this.regexs.REMOVE_BRACKET_TEXT, '$1').trim();
  }
  ...
  setDisplayText(content: string, displayText: string, linkTarget: string) {
    ...
    if (!linkLine) {
      this.log.warn("No link line found in content. Returning original content.");
      return content
    }
    const [beforeLink, afterLink] = content.split(linkLine[0])
    return beforeLink + (
      linkTarget ? `|link=${linkTarget}` : linkLine[1].trim()
    ) + (displayText !== linkTarget ? "|" + displayText : "") + afterLink
  }
